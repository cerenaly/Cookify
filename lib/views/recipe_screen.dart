...
import 'package:new_futter_app/controllers/recipe_controller.dart';
import 'package:new_futter_app/models/ingredient.dart';
import 'package:new_futter_app/models/nutrition.dart';
import 'package:new_futter_app/helpers/theme/app_theme.dart';
import 'package:new_futter_app/helpers/utils/my_utils.dart';
    ...
    if (controller.uiLoading) {
      ...
      return Scaffold(
        ...
        floatingActionButton: FloatingActionButton.extended(
          onPressed: () async {
            // Buraya test için sabit bir youtube linki veya controller'dan gelen video linkini koyabilirsin
            // Örn: controller.recipe.videoUrl
            final Uri url = Uri.parse('https://www.youtube.com/watch?v=dQw4w9WgXcQ');

            try {
              // launchUrl, eğer cihazda YouTube uygulaması varsa uygulamayı açar,
              // yoksa tarayıcı (Chrome/Safari) üzerinden videoyu başlatır.
              if (!await launchUrl(url, mode: LaunchMode.externalApplication)) {
                throw Exception('Could not launch $url');
              }
            } catch (e) {
              // Eğer bir hata oluşursa ScaffoldMessenger ile ekrana basıyoruz
              if (mounted) {
